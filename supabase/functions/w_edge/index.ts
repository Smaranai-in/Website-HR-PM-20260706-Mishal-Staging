...
case "create_internship_application": {
    ...
    const webAppUrl = Deno.env.get("GOOGLE_GSHEET_WEBAPP_URL") || "https://script.google.com/macros/s/AKfycbxJAL_Xw0KnMFJ4pY2foYnyEpQ2jLuK1O4CW5Pg2Nj59qQ710XkDe34wn638AQr1GZr/exec";
    if (webAppUrl) {
        try {
            const response = await fetch(webAppUrl, {
                ...
                body: JSON.stringify({
                    ...
                    secret: Deno.env.get("GSHEET_WEBHOOK_SECRET") || "smaranai-gsheet-webhook-secret-2026",
                    application: {
                        ...applicationData,
                        created_at: new Date().toISOString()
                    }
            ...
            if (response.ok && result.gsheet_id) {
                // Update the newly inserted record with the generated ApplicationID from the sheet!
                await admin
                    .from("w_internship_applications")
                    .update({ gsheet_id: result.gsheet_id })
                    .eq("id", appUuid);
                console.log(`[Backend Sync] Sync succeeded, updated gsheet_id: ${result.gsheet_id}`);
            } else {
                console.error("[Backend Sync] Web App returned error:", result.error || "Unknown error");
            }
        } catch (err) {
            console.error("[Backend Sync] Failed to connect to Web App:", err);
        }
    } else {
        // Fallback to old Google Form submission (Backend trigger)
...
/* ===== LEAVE MANAGEMENT ===== */
        ...
        case "apply_leave": {
            ...
            if (!leave_type || !start_date || !end_date || !reason) {
                return respond({ error: "leave_type, start_date, end_date, and reason are required" }, 400);
            }
        ...
        case "update_leave_status": {
            ...
            if (!["Approved", "Rejected"].includes(status)) {
                return respond({ error: "status must be 'Approved' or 'Rejected'" }, 400);
            }
        ...
        case "get_intern_tasks_and_projects": {
            const user = await requireAuth();

            const { data: projDevs, error: pdError } = await admin
                .from("p_project_developers")
                .select("project_id")
                .eq("developer_id", user.id);

            if (pdError) throw pdError;
            const pIds = projDevs?.map((pd: any) => pd.project_id) || [];

            let projects: any[] = [];
            let progressData: any[] = [];
            if (pIds.length > 0) {
                const [
                    { data: fetchedProjects, error: pError },
                    { data: fetchedProgress, error: progError }
                ] = await Promise.all([
                    admin.from("p_projects").select("*").in("id", pIds),
                    admin.from("p_calculate_project_progress").select("*").in("project_id", pIds)
                ]);
        ...
        case "get_intern_activity": {
            const user = await requireAuth();

            const { data: application, error: appError } = await admin
                .from("w_internship_applications")
                .select("id, full_name, top_priority_role, current_status, status_history, review, reviewtimestamp, created_at, cv_url")
                .eq("user_id", user.id)
                .order("created_at", { ascending: false })
                .limit(1)
                .maybeSingle();
            ...
            if (application) {
                const { data: asmData, error: asmError } = await admin
                    ...
                    .eq("user_id", user.id)
                    .eq("internship_application_id", application.id)
                    .maybeSingle();
        ...
        case "get_client_chat_admins_and_messages": {
            const user = await requireAuth();
        ...
        case "get_admin_chat_messages": {
            await requireAdmin();
            const { client_id } = body;

            if (!client_id) {
                return respond({ error: "client_id is required" }, 400);
            }
            ...
            let messages: any[] = [];
            if (adminIds.length > 0) {
                const { data, error: msgError } = await admin
                    .from("w_messages")
                    .select(`*, reply_to_message:w_messages!reply_to_id(*)`)
                    .or(`and(sender_id.eq.${client_id},receiver_id.in.(${adminIds.join(",")})),and(sender_id.in.(${adminIds.join(",")}),receiver_id.eq.${client_id})`)
                    .order("created_at", { ascending: true });

                if (msgError) throw msgError;
                messages = data || [];
            } else {
                return respond({ error: "No admins found" }, 404);
            }
        ...
        case "get_ai_interview_profile": {
            const user = await requireAuth();

            const [
                { data: internshipData, error: intError },
            ...
            return respond({
                success: true,
                cv_url: internshipData?.cv_url || null,
                resume_url: interviewData?.resume_url || null
            });
        }

        case "get_interview_record": {
            const user = await requireAuth();
            const { interviewId } = body;

            if (!interviewId) {
                return respond({ error: "interviewId is required" }, 400);
            }

            const { data: dbRecord, error } = await admin
                .from("interviews")
                .select("questions, answers, tab_switch_count, video_url, user_id")
                .eq("id", interviewId)
                .single();

            if (error) throw error;

            if (dbRecord.user_id !== user.id) {
                return respond({ error: "Forbidden" }, 403);
            }
