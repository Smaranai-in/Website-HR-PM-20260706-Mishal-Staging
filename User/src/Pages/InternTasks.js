...
export default function InternTasks() {
    ...
    useEffect(() => {
        async function fetchWork() {
            ...
            setLoading(true);
            try {
                const result = await callEdge("get_intern_tasks_and_projects");
                
                const myProjects = (result.projects || []).map(p => ({
                    ...p,
                    progress: result.progressData?.find(pr => pr.project_id === p.id) || { progress_percentage: 0 }
                }));
    ...
    const PRIORITY_CYCLE = ["all", "high", "medium", "low"];
    const cyclePriority = () => {
        const idx = PRIORITY_CYCLE.indexOf(filterPriority);
        setFilterPriority(PRIORITY_CYCLE[(idx + 1) % PRIORITY_CYCLE.length]);
    };
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button
                                    ...
                                    title={`Priority: ${filterPriority}`}
                                    className={`p-3 border rounded-2xl transition-all group relative ${
                                        filterPriority !== "all"
                                            ? "bg-blue-50 dark:bg-blue-900/30 border-blue-400 text-blue-600"
                                            : "bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-800 hover:border-blue-500"
                                    }`}
                                >
                                    <Filter className={`w-4 h-4 ${
                                        filterPriority !== "all" ? "text-blue-500" : "text-slate-500 group-hover:text-blue-500"
                                    }`} />
                                    {filterPriority !== "all" && (
                                        <span className="absolute -top-1.5 -right-1.5 text-[9px] font-black bg-blue-500 text-white rounded-full w-4 h-4 flex items-center justify-center capitalize">
                                            {filterPriority[0]}
                                        </span>
                                    )}
                                </button>
