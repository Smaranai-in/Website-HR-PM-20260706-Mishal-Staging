...
import { Moon, Sun, Menu, X, Sparkles, LogOut, MessageCircle } from "lucide-react";
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/90 dark:bg-[#0B1120]/95 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            {/* 2️⃣ NAV LINKS (Desktop) */}
            <nav className="hidden lg:flex items-center gap-1">
              {pages.map((page) => (
                <button
                  key={page.path}
                  onClick={() => navigate(page.path)}
                  className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium transition-all ${location.pathname === page.path
                ...
                {loginbool && profile ? (
                  <>
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium transition-all ${location.pathname === "/support-chat"
                        ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400"
                        : "text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800"
                        }`}
                    >
          ...
          <div className="px-4 py-6 space-y-4">
            ...
            <div className="space-y-1">
              {pages.map((page) => (
                <button
                  key={page.path}
                  onClick={() => { navigate(page.path); setMobileOpen(false); }}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all text-left ${location.pathname === page.path
                    ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400"
              ...
              {profile ? (
                <div className="space-y-3">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <button
                      onClick={() => { navigate("/support-chat"); setMobileOpen(false); }}
                      className={`w-full flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-semibold transition-colors ${location.pathname === "/support-chat"
                        ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400"
